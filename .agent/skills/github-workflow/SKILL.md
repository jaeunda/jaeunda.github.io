...
Before committing non-trivial work, choose validation commands that match the
changed surface. This repository uses `npm` scripts, not `pnpm` or Biome.

- Always run `git diff --check` for whitespace and patch hygiene.
- For TypeScript, component, config, Sass, or formatting-sensitive changes, run
  `npm run check`.
- For logic or utility changes covered by tests, run `npm test`.
- For content, layout, Quartz config, build pipeline, or broad UI changes, run
  `npm run quartz -- build`.

Report pass/fail status in the final handoff and include the same validation
list in the PR body. If a validation command is skipped because it is not
relevant, say why.
...
2. Create the GitHub issue (required for full workflow).
   - Use a conventional title matching the eventual commit and PR title.
   - Example: `chore(agents): unify shared skill workflow`
...
3. Create issue-numbered branch name and check out branch.
   - Branch format: `prefix<issue_num>/<short-kebab-summary>`
   - `prefix` should be the conventional commit type, such as `feat`, `fix`,
     `docs`, `style`, `refactor`, `test`, or `chore`.
   - Example: `feat123/team-po-fourth-deck`
...
7. Create the PR.
   ...
   - Use the same conventional title style as the issue and commit.
...
- Issue, commit, and PR title: conventional, short, outcome-oriented, and
  consistent with each other
  - `feat(<domain>): ...`
  - `fix(<domain>): ...`
  - `chore(<domain>): ...`
- Branch name: lowercase kebab-case summary with a type prefix immediately
  followed by the required issue number (`issue_num`), then a slash
  - `feat123/short-summary`
  - `fix456/short-summary`
  - `chore789/short-summary`
- Good branch examples:
  - `fix123/branding-svg-refresh`
  - `feat456/auth-profile-page`
  - `chore789/update-mock-handlers`
