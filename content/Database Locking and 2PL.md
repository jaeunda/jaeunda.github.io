...
- **Topological Sort**란 cycle이 없는 그래프에서 모든 edge $T_i \rightarrow T_j$ 에 대해 $T_i$ 가 $T_j$ 보다 앞에 오도록 순서를 매기는 것이다. 즉, Partial Order를 가지는 Transaction에 대하여 Partial Order를 만족하면서 전체 Transaction을 정렬하는 방식이다.
